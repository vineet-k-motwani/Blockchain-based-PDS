...
contract StateGov is Stakeholder {
  ...
  function register(
    ...
    require (_stakeholders[msg.sender].id ==  address(0), "StateGov::registerStateGov: StateGov already registered");
  ...
  function getStateGovRawProductDetails(address _id) public view returns (rawProduct[] memory){
  ...
  function getStateGov(address id) public view returns (
    stakeholder memory stateGov,
    bool isRenewableUsed,
    rawProduct[] memory rawProducts,
    uint256[] memory launchedProductIds
  ){
    stateGov = stakeholder(id, _stakeholders[id].name, _stakeholders[id].location, _stakeholders[id].role, _stakeholders[id].isVerified);
    isRenewableUsed = _isRenewableUsed[id];
    rawProducts = getStateGovRawProductDetails(id);
