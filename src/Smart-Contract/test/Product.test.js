...
contract('Product',(accounts)=>{
    ...
    it(`Adding Product with serialNo: ${productSerialNo}`, async ()=>{
        await productContract.addProduct(productSerialNo, "Product 1",["Cocoa, Milk"], {from: stateGovAddress});
        const product = await productContract.getProduct(productSerialNo);
        assert.isTrue(product.isValue);
        assert(product.ownership, stateGovAddress);
    })
    ...
    describe("Product Ownership", async ()=>{
        ...
        it("Updating Ownership", async ()=>{
            //transfering ownership of product from stateGov to distributer
            await productContract.updateOwnership(distributerAddress, productSerialNo, {from: stateGovAddress});
