...
import { fetchStateGov, formattedAddress } from '../../Services/Utils/stakeholder';
...
const Product = () => {
  ...
  const reload = async () => {
    ...
    const product = {
      ...
      "stateGov": await fetchStateGov(authState.address, contractState.stateGovContract, response.item["stateGov"])
    }
    setProduct(product);
  }
      ...
      <div className="row top-wrapper">
        ...
        <div className="col-12 col-md-8 tw-right">
          ...
          <span className='tw-brand'>
            Brand: {product.stateGov["name"]} &nbsp;| &nbsp;
            {product.stateGov.isRenewableUsed?
              <span className="">
