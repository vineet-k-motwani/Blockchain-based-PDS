...
const LaunchProduct = ({isModalOpen, toggleModal, stateGovRP}) => {
  ...
  const launch = async () => {
    ...
    const selectedRP = selectedRPIndexes.map(key => {
      return {
        "name": stateGovRP[key].name,
        "isVerified": stateGovRP[key].isVerified
      }
    })
    ...
    await contractState.stateGovContract.methods.launchProduct(product.id).send({from: authState.address});
      ...
      <Modal isOpen={isModalOpen} toggle={toggleModal}>
        ...
        <ModalBody>
          ...
          <div className="row mt-2 justify-content-around">
            {Object.keys(stateGovRP).map((rawProductIndex) => {
              const rawProduct = stateGovRP[rawProductIndex];
              return (
                <div className={`
