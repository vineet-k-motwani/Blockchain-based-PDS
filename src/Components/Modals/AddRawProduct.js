...
const AddRawProduct = ({isModalOpen, toggleModalOpen}) => {
  ...
  const getSuppliers = async () => {
    const suppliersAddress = await contractState.centralGovContract.methods.getRawProductCentralGovs(rawProduct.name).call({from: authState.address});
    const suppliers = {};
    const selectedSuppliers = {};
    for(var i =0; i<suppliersAddress.length; i++){
      suppliers[i] = await fetchCentralGov(authState.address, contractState.centralGovContract, suppliersAddress[i]);
  ...
  const add = async () => {
    ...
    await contractState.stateGovContract.methods.addRawProduct(rawProduct.name, selectedSuppliers).send({from: authState.address});
